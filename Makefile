#!/usr/bin/make
# USAGE: 'sudo make' to build a buster image (fernandolcouto/debian:buster).
# Define variables on the make command line to change behaviour
# e.g.
#       sudo make release=buster arch=amd64 tag=buster-amd64

# variables that can be overridden:
release ?= bullseye
prefix  ?= fernandolcouto
arch    ?= amd64
mirror  ?= http://httpredir.debian.org/debian/
tag     ?= $(release)-$(arch)

build: $(tag)/root.tar $(tag)/Dockerfile
	podman build -t $(prefix)/debian:$(tag) $(tag)
